...
- `TableSettings.MergeSplitTokens` (default **off**) merges two adjacent
  ...
  threshold word grouping uses, so a genuine column gutter — the `# Changelog
...
- Golden position parity is now asserted at **0.01pt on both axes**,
  ...
  column of a financial statement — is left alone. Cell bboxes are
  merged too, which matters when they drive a citation highlight.
